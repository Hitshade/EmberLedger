...
-- Row tooltip helpers kept outside UI.lua so the main dashboard file can stay focused
-- on frame construction, layout, and refresh work.
local M = {}
...
function M:AddProfessionCooldownTooltipLines(tooltip, charKey, profEntries)
    ...
    if not ok and EL.db and EL.db.settings and EL.db.settings.debug and EL.Print then
        EL:Print("Cooldown tooltip unavailable: " .. tostring(err))
    end
end
...
function M:ShowRowTooltip(row)
    ...
    GameTooltip:AddDoubleLine("Realm", (char and char.realm) or T("Unknown"), 0.82, 0.80, 0.72, 1, 1, 1)
    GameTooltip:AddDoubleLine("Last seen", self:FormatTooltipAgo(char and char.lastSeen), 0.82, 0.80, 0.72, 1, 1, 1)
    ...
    if row.profEntries and #row.profEntries > 0 then
        for i, prof in ipairs(row.profEntries) do
            ...
            if conc then
                ...
                GameTooltip:AddDoubleLine("   Alert threshold", tostring(thresholdValue) .. (hasOverride and " (override)" or " (global)"), 0.72, 0.72, 0.72, 1, 1, 1)
                ...
                elseif quantity >= thresholdValue then
                    ...
                    GameTooltip:AddDoubleLine("   Full in", EL:GetConcentrationFullIn(conc, now) or T("Unknown"), 0.72, 0.72, 0.72, 1, 1, 1)
                else
                    ...
                    GameTooltip:AddDoubleLine("   Ready at", self:FormatTooltipDate(now + readySeconds), 0.72, 0.72, 0.72, 1, 1, 1)
                    GameTooltip:AddDoubleLine("   Full in", EL:GetConcentrationFullIn(conc, now) or T("Unknown"), 0.72, 0.72, 0.72, 1, 1, 1)
                end
            else
            ...
            if moxie and type(moxie.quantity) == "number" then
                local moxieThreshold = EL.GetMoxieThreshold and EL:GetMoxieThreshold() or 600
    ...
    if EL:HasImbuedMulchAccess(row.mulchData) then
        ...
        GameTooltip:AddDoubleLine("Ready at", remain <= 0 and "Now" or self:FormatTooltipDate(readyAt), 0.72, 0.72, 0.72, 1, 1, 1)
    ...
    GameTooltip:AddLine((EL:IsCharacterPinned(row.charKey) and "Alt-click: unpin character" or "Alt-click: pin character"), 0.7, 0.7, 0.7)
